function [state_vector,total_infect_case] = SIS_temporalV2(DataSet,DataFlag,Infect_source,b,u)
%individual scale SIS simulation in temporal network 
%this version can calculate the total infectious case
...
elseif strcmp(DataFlag,'TimA')
    ...
    state_vector = zeros(timestamps+1,population); %store each individual's state at each timestep
    state_vector(1,:) = Infec_initial;
    total_infect_case = length(Infect_source);
    for t = 1:1:timestamps
...
%         rng(s);
        ...
        S = find(state_vector(t,:) == 0); %S represent susceptible individual in the previous timestamp
        ...
        total_infect_case = total_infect_case + length(S(RAN'<P'));
    end
end     
end